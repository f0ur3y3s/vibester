...
void GameState::update()
{
    ...
    if (currentState == GAME_PLAYING && !isPaused)
    {
        ...
        if (settings.itemsEnabled && GetRandomValue(0, 100) < settings.itemFrequency * 100 && GetFrameTime() *
            GetRandomValue(0, 1000) < 1)
        {
            spawnRandomItem();
        }
