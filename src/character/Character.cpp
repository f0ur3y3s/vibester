...
#include "attacks/StandardAttacks.h"
#include "attacks/AerialAttacks.h"
...
void Character::draw()
{
    ...
    // IMPORTANT: Always draw attack hitboxes to help debug the issue
    if (stateManager.isAttacking && !stateManager.isDying && !stateManager.isExploding)
    {
        for (auto& attack : attacks)
        {
            attack.draw(true); // Force debug mode to true to always show hitboxes
        }
    }

    // Add debug info about attack state
    if (stateManager.isAttacking)
    {
        const char* attackNames[] = {
            "NONE", "JAB", "FTILT", "UTILT", "DTILT", "DASH",
            "FSMASH", "USMASH", "DSMASH", "NAIR", "FAIR",
            "BAIR", "UAIR", "DAIR", "NSPECIAL", "SSPECIAL",
            "USPECIAL", "DSPECIAL", "GRAB", "PUMMEL", "FTHROW",
            "BTHROW", "UTHROW", "DTHROW"
        };

        int attackType = static_cast<int>(stateManager.currentAttack);
        if (attackType >= 0 && attackType < 24)
        {
            DrawText(
                attackNames[attackType],
                static_cast<int>(physics.position.x - 20),
                static_cast<int>(physics.position.y - height - 40),
                16,
                RED
            );
        }
        ...
        // Show attack frame
        char frameText[20];
        sprintf(frameText, "Frame: %d/%d", stateManager.attackFrame, stateManager.attackDuration);
        DrawText(
            frameText,
            static_cast<int>(physics.position.x - 30),
            static_cast<int>(physics.position.y - height - 60),
            12,
            RED
        );
    }
